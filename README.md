...
유튜브 영상을 원하는 해상도(mp4)나 음악 파일(mp3)로 저장하는 도구입니다.

## 두 가지 사용 방식

| 방식 | 동작 | 상태 |
| --- | --- | --- |
| **아이폰에서 직접 받기** (권장) | a-Shell 앱에서 `public/cnation-dl.sh` 실행. 아이폰이 직접 유튜브에 접속하므로 봇 차단이 발생하지 않습니다. | 바로 사용 가능 |
| **웹에서 바로 받기** | `/api/info`, `/api/download`가 서버에서 yt-dlp를 실행. 유튜브가 클라우드 IP를 차단하므로 주거용 프록시(`YTDLP_PROXY`) 설정이 필요합니다. | 프록시 설정 시 동작 |

배포된 페이지가 아이폰 설치 안내와 스크립트 배포처 역할을 합니다
(`https://<배포주소>/cnation-dl.sh`).
